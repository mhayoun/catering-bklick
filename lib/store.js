...
import { buildCatererHaystack, countOccurrences, getCodedFields } from './search';
...
export async function searchCaterers(filters) {
  ...
  // With a keyword search active, rank results by how many times the term appears across the
  // caterer's own fields and all of its packages - the more matches, the more relevant.
  const keyword = filters.keyword?.trim();
  if (!keyword) return filtered;
  return filtered
    .map((c) => ({ ...c, matchCount: countOccurrences(buildCatererHaystack(c), keyword) }))
...
function matches(c, f) {
  ...
  if (f.keyword) {
    if (countOccurrences(buildCatererHaystack(c), f.keyword) === 0) return false;
  }
