...
// Free-text field (string or {he,en,fr}) -> a single space-joined string covering every
// locale, so keyword search matches regardless of which language the visitor types in.
...
function catererSharedFields(c) {
...
function packageOwnFields(pkg) {
  return [
    localizedText(pkg.name),
    ...(pkg.eventTypes || []).map(normalizeCode),
    ...(pkg.addons || []).map((a) => localizedText(a.name)),
    ...Object.values(pkg.categoryItems || {}).flatMap((items) => (items || []).map((it) => localizedText(it)))
  ];
}

// Full searchable text for a caterer, across the caterer itself and every one of its packages.
export function buildCatererHaystack(c) {
  const parts = [...catererSharedFields(c), ...(c.packages || []).flatMap(packageOwnFields)];
...
// that package's own name/items/addons/event types.
export function buildPackageHaystack(c, pkg) {
  const parts = [...catererSharedFields(c), ...packageOwnFields(pkg)];
  return parts.filter(Boolean).join(' ').toLowerCase();
}
