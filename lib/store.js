...
// Free-text field (string or {he,en,fr}) -> a single space-joined string covering every locale,
// so keyword search matches regardless of which language the visitor types in.
...
// Constant codes are stored as snake_case (e.g. 'badatz_rav_machpud', 'bar_mitzvah') - turn the
// underscores into spaces so a keyword search for "Badatz Rav Machpud" or "Bar Mitzvah" matches.
function normalizeCode(code) {
  return String(code).replace(/_/g, ' ');
}

// Builds the full text a keyword search is matched against: every free-text field plus every
// coded field (kashrut, districts, catering/event types, menu categories, services) across the
// caterer AND its packages/formulas - not just business name/city/description.
function buildSearchHaystack(c, cDistricts, cKashrutLevels, cCateringTypes) {
  const parts = [
    c.businessName,
    localizedText(c.description),
    ...
    ...(c.packages || []).flatMap((p) => [
      localizedText(p.name),
      ...(p.eventTypes || []).map(normalizeCode),
      ...(p.addons || []).map((a) => localizedText(a.name))
    ])
  ];
...
function matches(c, f) {
  ...
  if (f.keyword) {
    ...
    if (!buildSearchHaystack(c, cDistricts, cKashrutLevels, cCateringTypes).includes(kw)) return false;
  }
