...
export async function listCaterers() {
  if (hasKv) {
    ...
    // One MGET round-trip instead of one GET per caterer - on every page load this used to mean
    // as many Redis REST calls as there are listings.
    const records = await kv.mget(...ids.map((id) => `caterer:${id}`));
