...
export async function upsertCaterer(data, ownerEmail) {
  ...
  const record = {
    ...
    // Every create or edit by the owner goes back to pending review -
    // status can only be changed via setCatererStatus() by an admin.
...
export async function deleteCaterer(id, ownerEmail) {
  const existing = await getCaterer(id);
...
function matches(c, f) {
  if (f.keyword) {
    ...
    const haystack = `${c.businessName} ${c.city} ${c.description}`.toLowerCase();
    if (!haystack.includes(kw)) return false;
  }
  if (f.districts?.length && !f.districts.includes(c.district)) return false;
  if (f.kashrutLevels?.length && !f.kashrutLevels.includes(c.kashrut)) return false;
  if (f.cateringTypes?.length && !f.cateringTypes.includes(c.cateringType)) return false;
  ...
  if (f.beverageTypes?.length) {
    const ok = f.beverageTypes.every((b) => (c.beverageTypes || []).includes(b));
    if (!ok) return false;
  }
